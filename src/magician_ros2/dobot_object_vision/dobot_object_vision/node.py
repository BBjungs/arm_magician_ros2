...
def convert_bundle(bundle, bridge, now, frame, max_age, max_skew, input_rectified=True):
    ...
    if not input_rectified:
        ...
        # Constant-zero borders would become a synthetic black contour and can
        # merge with a legitimate black target near the image edge. Replicate
        # the measured border instead; depth stays geometrically continuous
        # and the detector still rejects genuinely clipped target contours.
