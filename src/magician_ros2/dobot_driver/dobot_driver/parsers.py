...
def _parse_end_effector_state(payload):
    """Parse controller command state; this is not pressure feedback."""
    if len(payload) != 2 or any(value not in (0, 1) for value in payload):
        raise ValueError("End-effector response must contain two boolean bytes")
...
parsers = {
    ...
    # 63 uses the same payload schema but remains a distinct gripper ID.
    63: [_parse_end_effector_state, None, lambda x: struct.unpack('<Q', bytearray(x))[0], lambda x: list(struct.pack('<BB', *x))],
    # JOG
    ...
    # 246 GET: current queue index as uint64 (8 bytes).
