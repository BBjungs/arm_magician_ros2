...
class Interface:
    def __init__(self, port):
        ...
        self.port = port
        self.timeout = float(os.environ.get('DOBOT_SERIAL_TIMEOUT', '1.0'))
        lock_name = os.path.basename(os.path.realpath(port)) or 'dobot'
        lock_path = os.environ.get(
            'DOBOT_SERIAL_LOCK_PATH',
            f'/tmp/dobot-magician-{lock_name}.lock',
        )
        self._process_lock_fd = os.open(lock_path, os.O_CREAT | os.O_RDWR, 0o660)
        ...
        try:
            ...
            print(f"[WARN] Dobot Magician is not connected on {port}: {exc}")
            sys.exit(1)

    @contextmanager
    def _transaction(self):
        """Serialize a complete request across threads and ROS processes."""
        with self.lock:
    ...
    def send(self, message):
        with self._transaction():
            ...
            response = Message.read(
                ...
                expected_rw=message.rw,
                expected_is_queued=message.is_queued,
                timeout=self.timeout,
            )
        return None if response is None else response.params

    def send_only(self, message):
        """Send a command and consume its synchronous acknowledgement."""
        response = self.send(message)
        if response is None:
            raise TimeoutError(
                f"No valid acknowledgement for Dobot command ID {message.id}"
            )
        return response
    ...
    def get_current_queue_index(self):
        request = Message([0xAA, 0xAA], 2, 246, False, False, [], direction='out')
        return self.send(request)
