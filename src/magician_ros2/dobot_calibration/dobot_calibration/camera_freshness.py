...
def _stream_fresh(stream, lease_s):
    ...
    if not isinstance(stream, dict) or not stream.get('fresh', False):
        return False
    if not stream.get('current_timestamp_valid', stream.get('timestamp_valid', False)):
        return False
    if int(stream.get('current_duplicate_timestamps',
                      stream.get('duplicate_timestamps', 0))) != 0:
        return False
    if int(stream.get('current_regressed_timestamps',
                      stream.get('regressed_timestamps', 0))) != 0:
        return False
...
def evaluate_camera_freshness(*, now_s, bundle, streams, rgb_lease_s, depth_lease_s,
    ...
    # Retain the last *actual Image payload* capture through the configured
    # lease.  This prevents readiness sampling in the 10--30 ms interval
    # between RGB and depth callbacks from resetting the baseline timer.
    ...
    return CameraFreshness(rgb_ok, depth_ok, rgb_info_ok, depth_info_ok,
                           bool((sync_ok or recent_valid_sync)
                                and rgb_ok and depth_ok and rgb_info_ok),
