...
class DobotPublisher(Node):
    ...
    def timer_callback_alarms(self):
        with suppress(ValueError, TypeError, struct.error):
    ...
    def timer_callback(self):
        ...
        with suppress(ValueError, TypeError, struct.error):
            ...
            dobot_TCP.header.frame_id = 'magician_base_link'
            dobot_TCP.header.stamp = now.to_msg()
            dobot_TCP.pose.position.x = x/1000
            dobot_TCP.pose.position.y = y/1000
            dobot_TCP.pose.position.z = z/1000

            q = quaternion_from_yaw_degrees(r)
